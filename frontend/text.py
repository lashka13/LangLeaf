...
Если же хочешь оставить свой перевод, то напиши его через тире.
...
WORDS_ERROR = (
    "Что-то пошло не по плану((( Напиши в техподдержку и тебе помогут @otec_vpna"
)
...
DELETE_MARKUP.add(
    InlineKeyboardButton(text="💔 Удалить слова", callback_data="delete_all_words")
)

EDIT_BUTTON = InlineKeyboardBuilder()
EDIT_BUTTON.add(
    InlineKeyboardButton(text="Вернуться к изучению", callback_data="back_to_learn")
)
...
def get_menu_markup(swapped, cur_set):
    ...
    buttons.add(
        InlineKeyboardButton(
            text=f"🪓 Текущий сэт: {cur_set}", callback_data="change_set"
        )
    )
    buttons.add(
        InlineKeyboardButton(
            text=f"🌱 RU-EN: {'Да' if swapped else 'Нет'}", callback_data="swap_menu"
        )
    )
    return buttons
...
def get_sets_markup(sets):
    ...
    for set_group in sets:
        buttons.row(
            InlineKeyboardButton(
                text=set_group[0].title, callback_data=f"set_cur_state_{set_group[0].set_id}"
            )
        )
    buttons.row(
        InlineKeyboardButton(text="➕ Добавить новый сэт", callback_data="add_new_set")
    )
    return buttons
...
def get_learn_markup(word_id, flag, i):
    buttons = InlineKeyboardBuilder()
    buttons.row(
        InlineKeyboardButton(text="Следующее слово ➡️", callback_data="next_word")
    )
    buttons.row(
        InlineKeyboardButton(
            text="Удалить 😈", callback_data=f"delete_choice_{word_id}"
        ),
        InlineKeyboardButton(text="Свап 💫", callback_data="swap"),
        InlineKeyboardButton(
            text="🚩" if flag else "🏳️", callback_data=f"flag_{word_id}"
        ),
    )
    buttons.row(
        InlineKeyboardButton(text="❌ Остановиться", callback_data="stop_learning")
    )
    buttons.row(
        InlineKeyboardButton(text="🌎 Определения", callback_data=f"get_context_{i}")
    )
    buttons.row(
        InlineKeyboardButton(
            text="🐡 Скорректировать перевод", callback_data=f"edit_translate"
        )
    )
    return buttons
...
def get_delete_word_markup(word_id):
    ...
    DELETE_WORD_MARKUP.add(
        InlineKeyboardButton(text="Удалить", callback_data=f"delete_word_{word_id}")
    )
    DELETE_WORD_MARKUP.add(
        InlineKeyboardButton(text="Оставить", callback_data=f"next_word")
    )
...
NOTIFY_TEXT = [
    "Эй, не забывай прокачивать свои слова! Жми /learn.",
    "Пора встряхнуть свои знания! Давай, используй /learn.",
    "Как насчет того, чтобы повторить свои крутые слова? Введи /learn.",
    "Не упусти шанс стать мастером английского! Напиши /learn.",
    "Время для новых слов! Давай, используй /learn.",
    "Помни, что практика - это ключ к успеху! Используй /learn.",
    "Не откладывай на завтра то, что можешь выучить сегодня! Жми /learn.",
    "Давай, пора учить слова и удивлять всех! Используй /learn.",
    "Твои слова ждут, не заставляй их скучать! Используй /learn.",
    "Не забывай про обучение, это весело! Жми /learn.",
    "Слова сами себя не выучат! Действуй, используй /learn.",
    "Твой английский ждет, когда ты его прокачиваешь! Используй /learn.",
    "Пора учить и сиять! Введи /learn.",
    "Твой английский ждет, когда ты продолжишь! Напиши /learn.",
]

SET_TITLE_TEXT = "Напиши название для своего сэта"

SET_TITLE_ERROR = "Название сэта не должно превышать 150 символов"
